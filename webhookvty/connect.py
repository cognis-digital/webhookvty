"""Native cognis-connect emit for webhookvty — forward findings to any platform.

Maps webhookvty's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    webhookvty ... --format json | webhookvty-emit --to stix
    webhookvty-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('event', 'unknown')
        out['severity'] = 'info'
        out['type'] = 'webhook'
